  ...
  return (
    <div className="flex flex-col space-y-6 sm:space-y-8 bg-white max-w-7xl mx-auto w-full p-4 sm:p-8 rounded-[24px] shadow-sm border border-gray-100 pb-24 sm:pb-8">
      {/* Header with Back Button */}
       <div className="flex items-center gap-4">
         ...
         <div className="flex-1 min-w-0">
           <h1 className="text-xl sm:text-2xl poppins-bold text-gray-900 leading-none truncate">Edit Xendit</h1>
         </div>
       </div>
            ...
            <CardContent className="space-y-6 pt-0">
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <Input
                    value={
                      (currentContentIntegration?.configJson as XenditConfig)?.webhookVerif
                        ? `${typeof window !== "undefined" ? window.location.origin : ""}/api-backend/webhook/${(currentContentIntegration?.configJson as XenditConfig)?.webhookVerif}`
                        : ""
                    }
                    readOnly
                    variant="secondary"
                  ...
                  <button
                    onClick={() => {
                      const token = (currentContentIntegration?.configJson as XenditConfig)?.webhookVerif;
                      if (token) {
      ...
      <div className="sticky bottom-0 w-full z-50 px-0 sm:px-4">
        <div className="mx-auto max-w-5xl p-2 sm:p-5 bg-white/80 backdrop-blur-xl border border-white/50 shadow-2xl rounded-2xl sm:rounded-3xl flex justify-between items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
