...
const OtpModal: React.FC<{
  ...
  const handlePaste = (e: ClipboardEvent) => {
    ...
    if (data.length > 0) {
      const newOtp = [...otp];
      data.forEach((char, i) => {
        if (newOtp[i] !== undefined) newOtp[i] = char;
      });
      setOtp(newOtp);
    ...
    <BaseModal open={open} onClose={onClose}>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div className="relative w-full max-w-md rounded-2xl bg-white p-8 text-center shadow-2xl mx-4">
          ...
          <form
            onSubmit={(e) => handleSubmit(e, otp.join().replaceAll(",", ""))}
          >
            <div
              className="mb-8 flex justify-between gap-2"
              onPaste={handlePaste}
            >
              {otp.map((data, index) => (
                <input
            ...
            <button className="w-full rounded-xl bg-linear-to-r from-[#12D8BB] to-[#63F3E1] py-4 font-bold text-white shadow-lg shadow-cyan-200 transition-opacity hover:opacity-90">
              Verifikasi Kode
            </button>
          </form>
