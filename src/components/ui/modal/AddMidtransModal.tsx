...
interface AddMidtransModalProps {
  open: boolean;
  onClose: () => void;
}

export function AddMidtransModal({ open, onClose }: AddMidtransModalProps) {
  return (
    <BaseModal open={open} onClose={onClose}>
      <p className="text-lg mb-4 poppins-semibold">Tambahkan Midtrans Configuration</p>
      <div className="space-y-4">
        <Input placeholder="Name" variant="secondary" />
        <Input placeholder="Secret Key" variant="secondary" />
        <Input placeholder="Client Key" variant="secondary" />
